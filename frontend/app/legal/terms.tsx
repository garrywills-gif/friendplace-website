  ...
  return (
    <View style={{ flex: 1, backgroundColor: c.surface }}>
      ...
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 48, gap: 14 }}>
        <Text style={[styles.h1, { color: c.onSurface, fontSize: 26 * scale }]}>YouBelong Terms of Use</Text>
        ...
        <Text style={[styles.body, { color: c.onSurface, fontSize: 16 * scale }]}>
          Welcome to YouBelong — a warm, friendly place for friendship,
          connection and community. These Terms set out the simple rules that
          let us keep YouBelong a kind, safe space. By creating an account
          you agree to these Terms and our Privacy Policy.
        </Text>

        <Section title="Who can use YouBelong" c={c} scale={scale}>
        ...
        <Section title="Be kind. Always." c={c} scale={scale}>
          <Text style={[styles.body, { color: c.onSurface, fontSize: 16 * scale }]}>
            YouBelong has zero tolerance for harassment, hate speech,
        ...
        <Section title="This is a friendship community — not a dating app" c={c} scale={scale}>
          <Text style={[styles.body, { color: c.onSurface, fontSize: 16 * scale }]}>
            YouBelong exists to help people meet new friends, join local
        ...
        <Section title="Things you must not do" c={c} scale={scale}>
          ...
          <Bullet c={c} scale={scale}>Use YouBelong to advertise or sell goods or services without our written permission.</Bullet>
          <Bullet c={c} scale={scale}>Pretend to be a YouBelong staff member or a moderator.</Bullet>
        </Section>

        <Section title="Your content" c={c} scale={scale}>
          <Text style={[styles.body, { color: c.onSurface, fontSize: 16 * scale }]}>
            You own the photos and posts you share. By posting, you grant us a
            non-exclusive licence to display them inside YouBelong so other
        ...
        <Section title="No warranty / no liability" c={c} scale={scale}>
          <Text style={[styles.body, { color: c.onSurface, fontSize: 16 * scale }]}>
            YouBelong is provided “as is”. We work hard to keep it safe and
        ...
        <Section title="Changes to these Terms" c={c} scale={scale}>
          <Text style={[styles.body, { color: c.onSurface, fontSize: 16 * scale }]}>
            ...
            will be announced in-app. Continuing to use YouBelong after a
