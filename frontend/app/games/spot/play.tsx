...
function Scene({ elements, sceneW, sceneH, zoom, onTap, foundDiffs, hintCircle, testIDPrefix, backgroundUrl }: any) {
  ...
  return (
    ...
    >
      {/* Lifelike backdrop, generated by GPT Image 1 — sits underneath the
          interactive emoji "objects" so the scene feels like a real place
          while the game mechanic (find the changes among the objects) stays
          identical. */}
      {backgroundUrl ? (
        <Image
          source={{ uri: backgroundUrl }}
          pointerEvents="none"
          style={{ position: "absolute", left: 0, top: 0, width: sceneW, height: sceneH }}
          resizeMode="cover"
        />
      ) : null}
      <View pointerEvents="none" style={{ width: sceneW, height: sceneH, transform: [{ scale: zoom }] }}>
        {elements.map((el: Elem) => (
          <Text
            ...
            style={{
              position: "absolute",
              left: (el.x / 100) * sceneW - el.size / 2,
              top: (el.y / 100) * sceneH - el.size / 2,
              fontSize: el.size,
              // Subtle white halo so emoji stay legible on any backdrop.
