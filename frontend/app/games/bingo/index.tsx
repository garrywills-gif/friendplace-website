    ...
    <View style={{ flex: 1, backgroundColor: c.surface }}>
      ...
      <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={async () => { setRefreshing(true); await load(); setRefreshing(false); }} />} contentContainerStyle={{ padding: 14, paddingBottom: 60 }}>
        ...
        <Pressable testID="bingo-daily" onPress={() => start({ daily: true })} style={[styles.daily, { backgroundColor: "#1E3A7F", borderColor: c.brand }]}>
          ...
          <Text style={{ color: "#FFFFFFCC", fontSize: 14 * scale, marginTop: 2 }}>Same call sequence for everyone &middot; 15 Belong Points</Text>
        </Pressable>
            ...
            return (
              <Pressable key={d.key} testID={`bingo-diff-${d.key}`} onPress={() => setDiff(d.key)} style={[styles.diffRow, { backgroundColor: active ? `${d.color}22` : c.surfaceSecondary, borderColor: active ? d.color : c.border, borderWidth: active ? 2.5 : 1 }]}>
                ...
                <View style={{ flex: 1, marginLeft: 12 }}>
                  ...
                  <Text style={{ color: d.color, fontWeight: "800", fontSize: 12 * scale, marginTop: 2 }}>{d.points} Belong Points</Text>
                </View>
