...
/**
 * /admin/marketing/send — "Send Email" fast quick-send screen (iter159 P0).
 *
 * Designed for speed: minimum required = Name + Email. Everything else is
 * optional. Live preview updates as the admin types.
 *
 * Sending flow:
 *   1. Admin fills the form (name, email, optional template/flyer/subject/message).
 *   2. Live preview panel shows the fully personalised HTML.
 *   3. Admin clicks "Send" — POST /api/cms/marketing/send.
 *   4. On success, we clear the form (keep template selection) and toast.
 *
 * Privacy safeguard (matches backend invariant): this screen sends to a
 * SINGLE recipient. Bulk campaign delivery lives on a separate P1 screen
 * so nobody can accidentally paste ten addresses in here.
 */
...
export default function SendMarketingEmailPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setToast({ kind: 'err', msg: `Couldn’t load templates: ${e?.message || e}` });
      }
    })();
  }, []);
  ...
  useEffect(() => {
    ...
    if (ft && !ft.supported_layouts.includes(form.flyer_layout)) {
      setForm((prev) => ({ ...prev, flyer_layout: ft.default_layout }));
    }
      ...
      try {
        const flyerAttach = form.flyer_key
          ? {
              ...
              field_values: form.flyer_venue ? { venue: form.flyer_venue } : {},
            }
          : null;
  ...
  const canSend = form.template_id && form.recipient_email.includes('@') && !sending;
  const selectedFlyer = flyerTpls.find((f) => f.key === form.flyer_key);
  ...
  const send = async () => {
    ...
    try {
      ...
      if (r.ok) {
        setToast({ kind: 'ok', msg: `Sent to ${r.recipient_email}. Message id ${r.message_id}.` });
        setForm((prev) => ({
          ...INITIAL_STATE,
        ...
        setToast({ kind: 'err', msg: `Send failed: ${r.error || 'unknown error'} (${r.error_code || 'unknown'})` });
      }
    } catch (e: any) {
        ...
        <div style={formCol}>
          <div style={card}>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
              {(['person', 'organisation'] as const).map((v) => (
                <button
                  key={v}
                  type="button"
                  onClick={() => setForm({ ...form, recipient_type: v })}
                  style={{
                    ...pill,
                    background: form.recipient_type === v ? '#0D9488' : '#F1F5F9',
                    color:      form.recipient_type === v ? '#FFFFFF' : '#0F172A',
                    borderColor: form.recipient_type === v ? '#0D9488' : '#E2E8F0',
                  }}
                >
            ...
            {form.recipient_type === 'organisation' && (
              <>
                <label style={label}>Organisation name <span style={muted}>(required for org)</span></label>
                <input
                  style={input}
            ...
            <input
              ...
              placeholder={form.recipient_type === 'organisation' ? 'e.g. reception team' : 'e.g. Jane Smith'}
            />

            <label style={label}>Email address <span style={required}>*</span></label>
            <input
              style={input}
              type="email"
          ...
          <div style={card}>
            ...
            >
              {templates
                .filter((t) => t.audience === 'any' || t.audience === form.recipient_type)
                .map((t) => (
            ...
            <label style={label}>Subject <span style={muted}>(optional — template picks a default)</span></label>
            <input
              style={input}
              value={form.subject_override}
              onChange={(e) => setForm({ ...form, subject_override: e.target.value })}
              placeholder="Leave blank to use the template default"
            />

            <label style={label}>Additional message <span style={muted}>(optional)</span></label>
            <textarea
              style={{ ...input, minHeight: 120, resize: 'vertical', fontFamily: 'inherit' }}
              value={form.additional_message}
              onChange={(e) => setForm({ ...form, additional_message: e.target.value })}
              placeholder="Anything you’d like to add — blank lines create paragraphs."
            />
          </div>

          <div style={card}>
            <h3 style={cardTitle}>Flyer attachment <span style={muted}>(optional)</span></h3>

            <label style={label}>Flyer</label>
            <select
              style={input}
              value={form.flyer_key}
              onChange={(e) => setForm({ ...form, flyer_key: e.target.value })}
            >
              <option value="">— no flyer —</option>
              {flyerTpls.map((ft) => (
                <option key={ft.key} value={ft.key}>{ft.name}</option>
              ))}
            </select>
            ...
            {selectedFlyer && (
              <>
                ...
                <select
                  ...
                  onChange={(e) => setForm({ ...form, flyer_layout: e.target.value })}
                >
                ...
                <label style={label}>Venue / host <span style={muted}>(optional)</span></label>
                <input
                  style={input}
                  value={form.flyer_venue}
                  onChange={(e) => setForm({ ...form, flyer_venue: e.target.value })}
                  placeholder="e.g. Hillside Retirement Village"
                />
                <p style={helpText}>
                  The flyer will be attached as PDF, sized to {form.flyer_layout}.
                </p>
              </>
            )}
          </div>
          ...
          <div style={sendBar}>
            ...
            <Link href="/admin/marketing/history" style={historyLink}>View send history →</Link>
          </div>
          ...
          <div style={{ ...card, padding: 0, overflow: 'hidden' }}>
            <div style={previewHeader}>
              ...
              <span style={previewHeaderMeta}>
                {previewBusy ? 'refreshing…' : preview?.subject || '—'}
              </span>
            </div>
            {previewErr ? (
              ...
              <iframe
                title="Email preview"
                sandbox=""
                srcDoc={preview?.html || '<p style="padding:24px;color:#94A3B8;font-family:sans-serif;">Fill in the form to see your email…</p>'}
                style={{
                  width: '100%',
                  height: 720,
                  border: 'none',
            ...
            {preview?.flyer && (
              <div style={flyerMeta}>
                📎 Attachment: <strong>{preview.flyer.filename}</strong>{' '}
                <span style={muted}>({Math.round((preview.flyer.size_bytes || 0) / 1024)} KB PDF)</span>
              </div>
            )}
          </div>
        </div>
...
const helpText: React.CSSProperties = { margin: '6px 0 0', fontSize: 12, color: '#64748B', lineHeight: 1.5 };
...
const pill: React.CSSProperties = { padding: '8px 14px', borderRadius: 999, fontSize: 13, fontWeight: 700, border: '1px solid #E2E8F0', cursor: 'pointer' };
const sendBar: React.CSSProperties = { display: 'flex', alignItems: 'center', gap: 14, marginTop: 4 };
const sendBtn: React.CSSProperties = { background: '#0D9488', color: '#FFFFFF', border: 'none', borderRadius: 12, padding: '12px 28px', fontSize: 15, fontWeight: 800, letterSpacing: '0.02em' };
const historyLink: React.CSSProperties = { fontSize: 13, color: '#0F766E', fontWeight: 700, textDecoration: 'none' };
...
const flyerMeta: React.CSSProperties = { padding: '10px 16px', borderTop: '1px solid #E2E8F0', background: '#F8FAFC', fontSize: 13, color: '#334155' };
const muted: React.CSSProperties = { color: '#94A3B8', fontWeight: 500, fontSize: 11 };
