...
export default function MarketingHistoryPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await marketingApi.listSends({
          limit: 200,
        });
  ...
  return (
    <AdminShell title="Send History">
      <p style={intro}>
        <Link href="/admin/crm" style={crumb}>
          CRM
        </Link>
        {' › '}Marketing{' › '}Send History — every marketing email FriendPlace
        has sent, newest first. One row per recipient.
      </p>
